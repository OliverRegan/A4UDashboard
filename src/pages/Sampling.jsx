...
const Sampling = (props) => {
    ...
    useEffect(() => {
        if (audit.auditDetails.sampling.sampledTransactions.length > 0) {
            console.log(audit)
            if (audit.auditDetails.sampling.sampledTransactions[0].xeroInvoiceId && !hasXeroLink) {
                console.log("audit")
    ...
    }, [audit.auditDetails.sampling.sampledTransactions, refreshed])
    ...
    return (
        <div className='ml-5'>
            ...
            <div className='flex'>
                <div className={audit.auditDetails.accounts.selectedAccounts.length != 0 && refreshed ? 'w-1/2' : 'w-[100%]' + " min-h-full"}>
                    <AccountDetailsBar />
                </div>
